...
def populate_db():
  ...
  #Inital Users 
    user = User(name='Alice Smith', email='alice@example.com', password='password123')
    session.add(user)
    session.commit()

    user = User(name='Bob Johnson', email='bob@example.com', password='password123')
    session.add(user)
    session.commit()

    user = User(name='Charlie Brown', email='charlie@example.com', password='password123')
    session.add(user)
    session.commit()

    user = User(name='Diana Prince', email='diana@example.com', password='password123')
    session.add(user)
    session.commit()

    user = User(name='Edward Elric', email='edward@example.com', password='password123')
    session.add(user)
    session.commit()
